...
// Sem --single-process para evitar conflito entre dois Chromium
const puppeteerArgs = [
  '--no-sandbox',
...
function createClient(key) {
  ...
  c.on('disconnected', () => {
    console.warn(`⚠️  WhatsApp [${key}] desconectado. Reconectando em 15s...`);
    clients[key].ready = false;
    setTimeout(() => c.initialize(), 15000);
  });
...
// Inicializa os dois clientes com 20 segundos de intervalo
// para evitar conflito de processos do Chromium
