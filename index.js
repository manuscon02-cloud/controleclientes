...
app.post('/api/clients/:id/renew', (req, res) => {
  ...
  // Se vencido, renova a partir de hoje. Se ainda válido, soma 1 mês ao vencimento.
  const today = new Date(); today.setHours(0,0,0,0);
  const due = new Date(c.dueDate + 'T12:00:00'); due.setHours(0,0,0,0);
  const base = due < today ? today : due;
  base.setMonth(base.getMonth() + 1);
  res.json(db.update(c.id, { dueDate: base.toISOString().split('T')[0], status:'active', sentNotifications:[] }));
});
