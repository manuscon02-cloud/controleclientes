...
function authMiddleware(req, res, next) {
  ...
  // Checa token no header ou query
  const token = req.headers['x-auth-token'] || req.query.token;
  if (token && sessions.has(token)) return next();
  // Arquivos estáticos: deixa passar (o HTML vai checar via JS)
...
app.put('/api/servers/:id', (req, res) => {
  ...
  if (!updated) return res.status(404).json({ error: 'Servidor não encontrado.' });
  res.json(updated);
});

app.delete('/api/servers/:id', (req, res) => {
  if (!db.removeServer(req.params.id)) return res.status(404).json({ error: 'Servidor não encontrado.' });
...
app.post('/api/clients/:id/test', async (req, res) => {
  ...
  if (!clients[senderKey].ready) return res.status(503).json({ error: `WhatsApp "${clients[senderKey].label}" não está conectado.` });
  try {
    await clients[senderKey].instance.sendMessage(`${c.phone}@c.us`, `Olá, *${c.name}*! 👋\nEste é um teste do sistema de cobranças. Tudo funcionando! ✅`);
    res.json({ success: true });
...
app.get('/api/revenue', (req, res) => {
  ...
  res.json({ revenue, totalCost, profit, byServer, activeCount: active.length, totalCount: db.getAll().length });
});
