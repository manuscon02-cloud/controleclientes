...
app.post('/api/clients/:id/renew', (req, res) => {
  ...
  // Se vencido, renova 1 mês a partir de hoje. Se ainda válido, soma 1 mês ao vencimento atual.
  const baseDate = c.dueDate < todayStr ? new Date(todayStr) : new Date(c.dueDate + 'T12:00:00');
  baseDate.setMonth(baseDate.getMonth() + 1);
  res.json(db.update(c.id, { dueDate: baseDate.toISOString().split('T')[0], status:'active', sentNotifications:[] }));
});
